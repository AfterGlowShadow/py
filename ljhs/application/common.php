...
function PushGarbage($data, $res, $tkey, $validate)
{
    foreach ($data as $key => $value) {
        if (array_key_exists($validate, $value) && $value[$validate] != "") {
            if ((array_key_exists("number", $value) && $value['number'] != "") || (array_key_exists("weight", $value) && $value['weight'] != "")) {
                $data[$key][$tkey] = $res;
            } else {
                return false;
            }
...
/**
 * 获取报价
 * @param array $ids 垃圾分类
 * @param \think\Model $Model garbageprice
 */
function getGarbagePrice($ids = array(), $Model)
{
    ...
    $returnData = array();
    $ids_arr = explode(',', $ids);
//    $region = session('user')['userInfo']['region'];
    $region = 1;
    for ($i = count($ids_arr) - 1; $i >= 0; $i--) {
        $where = [];
        $where[] = ['garbageid', '=', $ids_arr[$i]];
        $where[] = ['starttime', 'lt', time()];
        $where[] = ['endtime', 'gt', time()];
        $where[] = ['regionz', '=', $region];
        $res = $Model->MFind($where, '', 'jprice,gprice,number,weight');
        if ($res) {
            $returnData = $res;
            break;
        }
